...
function PracticePanel({
  ...
  const totalMax = Object.values(results).reduce((s, r) => s + r.max_marks, 0);
  ...
  return (
    <div className="flex h-full flex-col">
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                ...
                {result && (
                  <span
                    ...
                    `}
                  >
                    {result.is_correct === true && <Check size={11} />}
                    {result.is_correct === false && <X size={11} />}
              ...
              {isChoice ? (
                <div className="space-y-1.5">
                  {q.options.map((o) => {
                    const selected = answer?.selected_index === o.index;
                    const isCorrect = result && q.correct_index === o.index;
                    const isWrongPick = result && selected && !isCorrect;
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          onChange={() =>
                            onAnswer({ question_id: q.id, selected_index: o.index })
                          }
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    onAnswer({ question_id: q.id, answer_text: e.target.value })
                  }
                  className="
                  ...
                  {result.rubric_breakdown.length > 0 && (
                    <details className="text-xs">
                      ...
                      <ul className="mt-2 space-y-1 pl-3">
                        {result.rubric_breakdown.map((r, j) => (
                          <li key={j} className="text-neutral-400">
                            <span
                              className={
                                r.awarded > 0 ? "text-emerald-400" : "text-neutral-600"
                              }
                            >
                              {r.awarded}/{r.max}
                            </span>{" "}
      ...
      <div className="shrink-0 border-t border-neutral-800 p-4">
        ...
        {marked > 0 && totalMax > 0 && totalMarks / totalMax < REVEAL_BELOW / 10 && (
          <p className="mt-2 text-center text-xs text-neutral-500">
            Model answers are shown above where you scored below half marks.
          </p>
        )}
      </div>
    </div>
