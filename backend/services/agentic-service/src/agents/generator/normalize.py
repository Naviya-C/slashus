...
Deliberately free of imports beyond the standard library. This is the layer
most likely to need a fix at 2am when a model starts returning something
slightly different, and it should be testable without a Groq key, a database,
or a 2GB embedding model loading first.
...
def normalize_questions(raw: list[dict], qtype: str) -> list[dict]:
    ...
    for i, q in enumerate(raw):
        ...
        item: dict[str, Any] = {
            ...
            "source_pages": [
                int(p) for p in q.get("source_pages", []) if str(p).isdigit()
            ],
        }

        built = (
            _build_choice(item, q) if qtype in MCQ_TYPES else _build_written(item, q)
        )
...
def _build_choice(item: dict, q: dict) -> dict | None:
    ...
    # whole set.
    if len(options) < 2:
        logger.warning("dropping MCQ with %d options", len(options))
        return None
    ...
    if not 0 <= idx < len(options):
        logger.warning("dropping MCQ with out-of-range correct_index %r",
                       q.get("correct_index"))
        return None
...
def _build_written(item: dict, q: dict) -> dict | None:
    ...
    if not rubric:
        logger.warning("dropping written question with no rubric")
        return None
